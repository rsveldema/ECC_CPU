	...
	public:
		...
		uint32_t getEncodedInstruction() override
		{
			...
			case Type::LO:
			{
				address >>= 0;
				address &= mask;
				return ((uint32_t)ecc::Opcode::INSN_OPCODE_MOVE_R0_CONST24A) | ((uint32_t)address << 8);
			}
			case Type::MID: {
				address >>= 24;
				address &= mask;
				return ((uint32_t)ecc::Opcode::INSN_OPCODE_MOVE_R0_CONST24B) | ((uint32_t)address << 8);
			}

			case Type::HI: {
				address >>= 48;
				address &= mask;
				return ((uint32_t)ecc::Opcode::INSN_OPCODE_MOVE_R0_CONST24C) | ((uint32_t)address << 8);
			}
			}
			return 0;
		}
